...
function BottomMenu() {
    return (
        <div className='absolute px-4 py-2 pb-3 bottom-0 left-0 right-0 bg-[#FFFFFF] shadow-[0_-2px_8px_rgba(0,0,0,0.1)] z-auto'>
                ...
                {link.map(({ name, icon, href }) => (
                    <li key={name} >
                        <Link href={href} className='flex flex-col align-middle text-[12px] text-[#757581]'>
                            <span className='text-[16px] mb-1 flex justify-center'>
                                <Image className='text-sm' src={icon} alt={name} width={24} height={24} />
                            </span>
                            {name}
                        </Link>
                    </li>
